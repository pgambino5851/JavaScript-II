...
largeShirts = runners.filter(function(currentValue){
   if(currentValue.shirt_size.includes('L')){
       return currentValue;
   } 
});
...
let govtWorkers = [];
govtWorkers = runners.filter(function(currentValue){
    if(currentValue.email.includes('.gov')){
        return currentValue.first_name + " " + currentValue.last_name;
    }
});
console.log(govtWorkers);
// Problem 2

let sortedByDonation = [];
let bestDonors = [];

sortedByDonation = runners.filter(function(currentValue){
    if(currentValue.donation > 200){
        return sortedByDonation.push(currentValue);
    }
})

sortedByDonation.sort(function(a, b){
    return a.donation - b.donation;
});

sortedByDonation.forEach(function(currentValue){
    bestDonors.push(`${currentValue.first_name} ` + `${currentValue.last_name}, $${currentValue.donation}`);
});

console.log(bestDonors);

// bestDonors = sortedByDonation.forEach(function(currentValue){
//     bestDonors.push(`${currentValue.first_name} ` + `${currentValue.last_name}, $${currentValue.donation}`);
// });
...
let lastNamesThatStartWithT = [];
lastNamesThatStartWithT = runners.filter(function(currentValue){
    if(currentValue.last_name.charAt(0) === 'T'){
        return currentValue;
    }
});


console.log(lastNamesThatStartWithT);